...
This integral looks innocent, but it is notoriously stubborn. There is no elementary antiderivative for $\frac{\sin x}{x}$, and the function oscillates all the way to infinity.
...
And then it gets even more remarkable. Differentiating under the integral sign gives "Cauchy's Integral Formula for Derivatives":
